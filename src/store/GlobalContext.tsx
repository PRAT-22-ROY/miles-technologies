...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Global Shift Tick (used purely to force re-renders, actual duration calculated in components)
  const [shiftTick, setShiftTick] = useState(0);
  const [shiftStatus, setShiftStatusInternal] = useState("offline");

  useEffect(() => {
    const interval = setInterval(() => {
      setShiftTick((prev) => prev + 1);
    }, 1000);
  ...
  const getShiftDuration = () => {
    let startTime = null;
    if (activeMode === "agent" && currentAgent?.clockInTime)
      startTime = new Date(currentAgent.clockInTime);
    if (activeMode === "employee" && currentEmployee?.clockInTime)
      startTime = new Date(currentEmployee.clockInTime);
    if (activeMode === "driver" && currentEmployee?.clockInTime)
      startTime = new Date(currentEmployee.clockInTime);
  ...
  const startShift = () => {
    ...
    if (activeMode === "agent" && currentAgent)
      setCurrentAgent({
        ...currentAgent,
        status: "online",
    ...
    if (
      (activeMode === "employee" || activeMode === "driver") &&
      currentEmployee
    )
      setCurrentEmployee({
        ...currentEmployee,
  ...
  const endShift = (agentOverride?: Agent, employeeOverride?: Employee) => {
    setShiftStatusInternal("offline");
    const now = new Date();
    ...
    if (activeMode === "agent" && currentAgent && currentAgent.id === aId)
      setCurrentAgent({ ...currentAgent, status: "offline" });
    if (
      (activeMode === "employee" || activeMode === "driver") &&
      currentEmployee &&
      currentEmployee.id === eId
    )
      setCurrentEmployee({ ...currentEmployee, status: "offline" });
  };

  const logout = () => {
    endShift(currentAgent || undefined, currentEmployee || undefined);
    ...
    if (activeMode === "driver") setCurrentEmployee(null);
  };
  ...
  const endChatCustomer = (ticketId: string) => {
    ...
    // Don't set null here immediately so they can see the feedback screen
  };
  ...
  const submitTimesheet = (
    ...
    setDb((prev) => ({ ...prev, timesheets: [newTs, ...prev.timesheets] }));
  };

  const updateTimesheetStatus = (tsId: string, status: any) => {
    setDb((prev) => ({
      ...prev,
      timesheets: prev.timesheets.map((t) =>
        t.id === tsId ? { ...t, status } : t,
      ),
    }));
  };

  const updateEmployeeProjectCode = (empId: string, projectCode: string) => {
    setDb((prev) => ({
      ...prev,
      employees: prev.employees.map((e) =>
        e.id === empId ? { ...e, projectCode } : e,
      ),
    }));
  };
